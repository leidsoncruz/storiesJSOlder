...
(function (global) {
  const ZuckJS = (function () {
    const w = window;

    const ZuckJS = function ZuckJS(timeline, options) {
      const d = document;
      const zuck = this;
      ...
      /* core functions */
      const query = function query(qs) {
        return d.querySelectorAll(qs)[0];
      };

      const get = function get(array, what) {
        if (array) {
          return array[what] || '';
        }
        return '';
      ...
      const each = function each(arr, func) {
        if (arr) {
      ...
      const setVendorVariable = function setVendorVariable(ref, variable, value) {
        const variables = [
      ...
      const addVendorEvents = function addVendorEvents(el, func, event) {
        const events = [
          event.toLowerCase(),
          `webkit${event}`,
          `MS${event}`,
      ...
      const onAnimationEnd = function onAnimationEnd(el, func) {
        addVendorEvents(el, func, 'AnimationEnd');
      };

      const onTransitionEnd = function onTransitionEnd(el, func) {
      ...
      const prepend = function prepend(parent, child) {
      ...
      const getElIndex = function getElIndex(el) {
      ...
      const fullScreen = function fullScreen(elem, cancel) {
      ...
      const translate = function translate(element, to, duration, ease) {
      ...
      const findPos = function findPos(obj, offsetY, offsetX, stop) {
        let curleft = 0;
        let curtop = 0;
      ...
      const timeAgo = function timeAgo(time) {
      ...
      /* modal */
      const zuckModal = function zuckModal() {
        const opened = false;
        ...
        const moveStoryItem = function moveStoryItem(direction) {
        ...
        var createStoryViewer = function createStoryViewer(
          ...
          each(get(storyData, 'items'), (i, item) => {
            ...
            const commonAttrs = `data-index="${i}" data-item-id="${get(item, 'id')}"`;
          ...
          const addMuted = function addMuted(video) {
            if (video.muted) {
        ...
        const createStoryTouchEvents = function createStoryTouchEvents(
          modalSliderElement,
        ) {
          const modalContainer = query('#zuck-modal');
          ...
          const touchStart = function touchStart(event) {
          ...
          var touchEnd = function touchEnd(event) {
            ...
            if (delta) {
              ...
              const duration = touchOffset
                ...
                : undefined;
              const isValid = (Number(duration) < 300 && Math.abs(delta.x) > 25)
            ...
            if (nextTimer) {
              ...
              const navigateItem = function navigateItem() {
                if (
        ...
        return {
          show: function show(storyId, page) {
            ...
            const callback = function callback() {
              ...
              const tryFullScreen = function tryFullScreen() {
                if (
      ...
      /* parse functions */
      const parseItems = function parseItems(story) {
      ...
      const parseStory = function parseStory(story) {
        const storyId = story.getAttribute('data-id');
        let seen = false;
      ...
      var playVideoItem = function playVideoItem(elements, unmute) {
        ...
        if (itemElement.getAttribute('data-type') == 'video') {
          ...
          const setDuration = function setDuration() {
            if (video.duration) {
              setVendorVariable(
      ...
      const getLocalData = function getLocalData(key) {
        if (option('localStorage')) {
          const keyName = `zuck-${id}-${key}`;
      ...
      zuck.navigateItem = zuck.nextItem = function (direction, event) {
        ...
        if (storyViewer && nextPointer && nextItem) {
          const navigateItemCallback = function navigateItemCallback() {
